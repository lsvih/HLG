...
def convert_examples_to_features(examples, label_list, max_seq_length):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # 构建节点对
        # g_c = graph_character, g_w = graph_word
        g_c, g_w = [], []
        word_idx = 0
...
class GraphTensorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.input_ids[idx], self.graphs[idx], self.input_mask[idx], self.char_mask[idx], self.word_mask[idx], \
...
def collate(batch):
    ...
    input_ids, batch_graph, input_mask, char_mask, word_mask, sentence_mask, label_ids = batch_data
    ...
    return torch.stack(input_ids), None, torch.stack(batch_c2w), \
